...
void printIntArrCurrentState(void* arr, size_t curI, size_t count, bool isForward) {
    for (int j = 0; j < count; ++j) {
        int el = *(const int*) (arr + (j * sizeof(int)));
        if (j == curI) {
            if (isForward) {
                printf("➡️%d ", el);
            } else {
...
int16_t ECOCALLMETHOD CEcoLab1Sink_OnContinueLog(/* in */ struct IEcoLab1Events* me, void* arr, size_t i, size_t count, bool isForward) {
    
    printIntArrCurrentState(arr, i, count, isForward);
...
IEcoLab1VTblEvents g_x2D2E3B9214F248A6A09ECB494B59C795VTblEvents = {
    ...
    CEcoLab1Sink_OnSwap,
    CEcoLab1Sink_OnContinueLog,
    CEcoLab1Sink_OnCompare
};
